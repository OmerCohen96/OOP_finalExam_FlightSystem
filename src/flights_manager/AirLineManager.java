...
//                getFlightsMap().put(flight.getFlight_code(), new ArrayList<>());
    ...
    public AirLine getAirLineComponent(String comp){
        return this.getAirLinesPool().stream().
    ...
    public AirLine getGroup(String compName){
        return this.getAirLineGroups().stream().filter(
                airLine -> airLine.getCompanyName().equalsIgnoreCase(compName)).findFirst().orElse(null);
    }

    public List<AirLine> getAirLinesPool(){
        List<AirLine> allAirLines =
                getAirLineGroups().stream().flatMap(airLine -> airLine.getAllCompanies().stream()).distinct().toList();
        return allAirLines;
    }
...
//    }
    ...
    public static void main(String[] args) {
        ...
        manager.addNewFlight(flight1,flight2,flight3,flight4,flight5,flight6,flight7,flight8,flight9,flight10
                                ,flight11,flight12,flight13,flight14,flight15,flight16,flight17,flight18,flight19,flight20,flight21);
